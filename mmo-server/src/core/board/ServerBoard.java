...
public class ServerBoard implements Board {
	...
	public ServerBoard(int width, int height){
		...
		for(int y = 0; y < height; y++)
			for(int x = 0; x < width; x++)
				cells[y * width + x] = new ServerCell(this, x, y);
    ...
    public Buff addBuff(String id, int timesToRepeat, int period, int initialDelay, Object data){
        ...
        Buff buff = new ServerBuff(this, buffData, timesToRepeat, period, initialDelay, data);
        buffs.add(buff);
        return buff;
    }
		...
		for(int y = area.y; y < area.y + area.height; y++)
			for(int x = area.x; x < area.x + area.width; x++){
				if (getCell(x, y).getContentsType() == Cell.CONTENTS_EMPTY){
					...
					Unit unit = new ServerUnit(cardMaster, unitData, getCell(x, y));
